...
- **网格描述**：块（Blocks）由一系列添加/移除操作定义。层（Layers）由一系列块和层的厚度定义。面或边界由特定的字符串表示定义，格式为 ``Face|Direction|LayerIndex|X1_min,X1_max,Y1_min,Y1_max;X2_min,X2_max,Y2_min,Y2_max;...``，例如，`Z|E|0|0,50,50,100;50,100,0,50;50,100,50,100`。
