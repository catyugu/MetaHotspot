...
class FVMSolver:
    def __init__(self, config_path):
        ...
        self.tag_to_mat = {
            t: m
            for m, tags in self.config.get("domain_material_assignment", {}).items()
            for t in tags
        }
        self.cells = []
        self._prepare_mesh()
    ...
    def _prepare_mesh(self):
        ...
        for i, nodes in enumerate(hex_data):
            p = points[nodes]
            p_min, p_max = np.min(p, axis=0), np.max(p, axis=0)
            ...
            self.cells.append(
                {
                    ...
                    "dims": p_max - p_min,
                    "box": np.concatenate([p_min, p_max]),
    ...
    def assemble_g_matrix(self):
        ...
        z_coords = sorted(list(set(round(c["center"][2], 10) for c in self.cells)))
        layer_cells = {
            z: [c for c in self.cells if round(c["center"][2], 10) == z]
            for z in z_coords
        }
        ...
        for idx, z in enumerate(z_coords):
            ...
            for i, c1 in enumerate(cells):
                for j in range(i + 1, len(cells)):
                    c2 = cells[j]
                    dist = np.abs(c1["center"] - c2["center"])
                    axis = np.where(dist > 1e-9)[0]
                    if (
                        len(axis) == 1
                        and dist[axis[0]]
                        < (c1["dims"][axis[0]] + c2["dims"][axis[0]]) / 2 + 1e-7
                    ):
                        ax = axis[0]
                        area = c1["dims"][(ax + 1) % 3] * c1["dims"][(ax + 2) % 3]
                        G = 1.0 / (
                            (c1["dims"][ax] / 2) / (c1["k"] * area)
                            + (c2["dims"][ax] / 2) / (c2["k"] * area)
                        )
                        rows.extend([c1["id"], c2["id"], c1["id"], c2["id"]])
                        cols.extend([c1["id"], c2["id"], c2["id"], c1["id"]])
                        data.extend([-G, -G, G, G])
            # 2. Inter-layer
            if idx < len(z_coords) - 1:
                ...
                for c1 in cells:
                    if (
                        abs(
                            c1["box"][5]
                            - (z_coords[idx + 1] - next_cells[0]["dims"][2] / 2)
                        )
                        < 1e-8
                    ):
                        for c2 in next_cells:
                            ...
                            if area > 1e-11:
                                G = 1.0 / (
                                    (c1["dims"][2] / 2) / (c1["k"] * area)
                                    + (c2["dims"][2] / 2) / (c2["k"] * area)
                                )
                                rows.extend([c1["id"], c2["id"], c1["id"], c2["id"]])
                                cols.extend([c1["id"], c2["id"], c2["id"], c1["id"]])
                                data.extend([-G, -G, G, G])
    ...
    def solve(self):
        ...
        if os.path.exists(ptrace_path):
            with open(ptrace_path, "r") as f:
                ...
                # p_raw is Power in Watts. We might need to scale it if TraceValue has other meaning.
                # Here we assume TraceValue = Power (W).
                ptrace_steps = [
                    dict(zip(header, [float(x) for x in l.split()]))
                    for l in f
                    if l.strip()
                ]

        G = self.assemble_g_matrix()
        n = len(self.cells)
        b_bc = np.zeros(n)
        r_bc, c_bc, d_bc = [], [], []
        ...
        if self.config.get("simulation_type") == "steady":
            ...
            p_avg = (
                {
                    u["name"]: np.mean([s[u["name"]] for s in ptrace_steps])
                ...
                for u in self.config["power_units"]:
                    inter = np.prod(
                        np.maximum(
                            ...
                            - np.maximum(c["box"][:3], [u["lx"], u["ly"], u["lz"]]),
                        )
                    )
                    if inter > 1e-15:
                        b[c["id"]] += (
                            inter / (u["dx"] * u["dy"] * u["dz"])
                        ) * p_avg.get(u["name"], 0.0)
            ...
            dt = self.config.get("timestep", 0.1)
                ...
                for c in self.cells:
                    for u in self.config["power_units"]:
                        ...
                        if inter > 1e-15:
                            b[c["id"]] += (
                                inter / (u["dx"] * u["dy"] * u["dz"])
                            ) * p_step.get(u["name"], 0.0)
                T = splinalg.spsolve(A_mat, b)
                if step % 10 == 0 or step == len(ptrace_steps) - 1:
                    print(
                        f"[STEP {step:4d}] T_min={np.min(T):.2f} K, T_max={np.max(T):.2f} K"
                    )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage: python solver.py <config.toml>")
    else:
        FVMSolver(sys.argv[1]).solve()
