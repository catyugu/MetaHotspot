...
def projected_static_residual(
    Kii: sp.csc_matrix, Kip: sp.csc_matrix, W: sp.csc_matrix, block_size: int
) -> float:
    Krr = (W.T @ Kii @ W).tocsc()
    lu = spla.splu(Krr)
...
def evaluate(
    data: Data, cfg: Package, run: Run, W: sp.csc_matrix, h: float, ref: Reference
):
    ...
    def run_solve(transient: bool):
        ...
        started = time.perf_counter()
        with solve_macro(
            compiled, model, ports_, state, options(run, transient)
        ) as solution:
    ...
    return {
        ...
        "projection_s": reduced.projection_s,
        ...
        "reduced_macro_order": int(reduced.K.shape[0]),
        "reduced_online_order": int(detail_n + reduced.K.shape[0]),
        "reduced_macro_k_nnz": int(reduced.K.nnz),
        "reduced_macro_c_nnz": int(reduced.C.nnz),
