...
def column_basis(compiled, cfg, base, delta, ports, dynamic_modes, shift_multipliers):
    ...
    for ix in range(compiled.nx):
        for iy in range(compiled.ny):
            ...
            if port is not None:
                ...
                sensitivity_rhs = dK_ii[cells][:, cells] @ static + dK_ip[cells, port].toarray().ravel()
                if np.linalg.norm(sensitivity_rhs) > 1.0e-14 * max(np.linalg.norm(b), 1.0):
                    candidates.append(
                        scipy.linalg.solve(k, -sensitivity_rhs, assume_a="sym", check_finite=False)
                    )
    ...
    initial_internal = np.asarray(basis.T @ np.full(basis.shape[0], cfg.ambient_K)).ravel()
...
def run_experiment(cfg, boundaries, strict, dynamic_modes, shift_multipliers):
    ...
    with ExitStack() as stack:
        ...
        detail_transient = build_model(cfg, Study.TRANSIENT, detail=True, macro=False).compile()
        ...
        if combined.size != full_layout.cell_count or np.unique(combined).size != combined.size:
        ...
        for convection_h in boundaries:
            ...
            started = time.perf_counter()
            reduced = affine_operators(reduced_base, reduced_delta, convection_h / cfg.affine_anchor_h)
            ...
            if times.shape != reference_times.shape or not np.allclose(times, reference_times, atol=1.0e-12, rtol=0.0):
        ...
        return {
            ...
            "passed": bool(compression_passed and all(result["passed"] for result in results)),
        }
...
def main(argv=None) -> int:
    ...
    print("=" * 96)
    print(f"Grid target: max XY cell={cfg.max_xy_cell_mm:g} mm, vertical cells={cfg.nz}")

    report = run_experiment(cfg, BOUNDARIES, args.strict, LOCAL_DYNAMIC_MODES, BDF1_SHIFTS)
