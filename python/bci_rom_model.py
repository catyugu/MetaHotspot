...
class Package:
    ...
    def total_height_mm(self) -> float:
        return sum((self.substrate_mm, self.bump_mm, self.die_mm, self.tim_mm, self.spreader_mm, self.cold_plate_mm))
...
def add_materials(model) -> None:
    for args in (('organic', '.65', '.65', '.55', '1900', '1100'), ('underfill', '.8', '.8', '.8', '1550', '1000'), ('copper', '390', '390', '390', '8960', '385'), ('mold', '.85', '.85', '.75', '1850', '1000'), ('silicon', '130', '130', '115', '2330', '700'), ('tim', '4', '4', '3', '2500', '900'), ('aluminum', '180', '180', '180', '2700', '900')):
        model.add_material(*args)

def full_rect(model, block: int, cfg: Package) -> None:
    model.add_rect(block, GeometryOp.ADD, '0', '0', str(cfg.width_mm), str(cfg.height_mm))
...
def build_package(cfg: Package, run: Run, include_macro: bool, study: Study, h=None):
    ...
    detail_layers = ((cfg.substrate_mm, cfg.substrate_cells), (cfg.bump_mm, cfg.bump_cells), (cfg.die_mm, cfg.die_cells))
    macro_layers = ((cfg.tim_mm, cfg.tim_cells), (cfg.spreader_mm, cfg.spreader_cells), (cfg.cold_plate_mm, cfg.cold_plate_cells))
    model.set_settings(study=study, length_unit=LengthUnit.MILLIMETER, initial_temperature_K=cfg.ambient_K, duration=run.duration_s if study == Study.TRANSIENT else 0.0, output_interval=run.dt_s if study == Study.TRANSIENT else 0.0)
    layers = (*detail_layers, *macro_layers) if include_macro else detail_layers
    model.set_mesh(vertices(cfg.width_mm, cfg.nx), vertices(cfg.height_mm, cfg.ny), z_vertices(layers))
    add_materials(model)
    if study == Study.TRANSIENT:
        t = run.duration_s
        model.add_function_piecewise('power_scale', np.asarray(((0.0, 0.0), (0.12 * t, 1.0), (0.42 * t, 0.65), (0.58 * t, 1.15), (0.82 * t, 0.35), (t, 0.85))))
    source = f'{power_density(cfg):.17g}' + ('*power_scale(x)' if study == Study.TRANSIENT else '')
    if include_macro:
        for thickness, material in ((cfg.cold_plate_mm, 'aluminum'), (cfg.spreader_mm, 'copper'), (cfg.tim_mm, 'tim')):
    ...
    for x, y in chiplets(cfg):
        ...
        model.add_rect(block, GeometryOp.ADD, str(x), str(y), str(cfg.chiplet_width_mm), str(cfg.chiplet_height_mm))
    ...
    for iy in range(cfg.bump_rows):
        for ix in range(cfg.bump_columns):
            ...
            model.add_rect(block, GeometryOp.ADD, str(x), str(y), str(cfg.bump_width_mm), str(cfg.bump_width_mm))
    ...
    if include_macro and h is not None:
        model.add_convection(str(h), str(cfg.ambient_K), [(Axis.Z, cfg.total_height_mm, 0, cfg.width_mm, 0, cfg.height_mm)])
    return model

def build_macro(cfg: Package, h=None):
    model = metahotspot.Model()
    layers = ((cfg.tim_mm, cfg.tim_cells), (cfg.spreader_mm, cfg.spreader_cells), (cfg.cold_plate_mm, cfg.cold_plate_cells))
    model.set_settings(study=Study.STEADY, length_unit=LengthUnit.MILLIMETER, initial_temperature_K=cfg.ambient_K)
    model.set_mesh(vertices(cfg.width_mm, cfg.nx), vertices(cfg.height_mm, cfg.ny), z_vertices(layers))
    add_materials(model)
    for thickness, material in ((cfg.cold_plate_mm, 'aluminum'), (cfg.spreader_mm, 'copper'), (cfg.tim_mm, 'tim')):
        layer = model.add_layer(str(thickness))
    ...
    model.set_default_neumann('0')
    if h is not None:
        model.add_convection(str(h), str(cfg.ambient_K), [(Axis.Z, sum((x[0] for x in layers)), 0, cfg.width_mm, 0, cfg.height_mm)])
    return model

def patches(cfg: Package, face: Face, z: float) -> list[PortPatch]:
    dx, dy = (cfg.width_mm * 0.001 / cfg.nx, cfg.height_mm * 0.001 / cfg.ny)
    return [PortPatch(int(face), z, (i * dx, (i + 1) * dx, j * dy, (j + 1) * dy)) for i in range(cfg.nx) for j in range(cfg.ny)]
...
def grid_cells(compiled, z0: int, z1: int) -> np.ndarray:
    return np.asarray([int(compiled.grid_to_cell[(i * compiled.ny + j) * compiled.nz + k]) for i in range(compiled.nx) for j in range(compiled.ny) for k in range(z0, z1)])

def macro_columns(compiled) -> tuple[np.ndarray, ...]:
    columns = []
    for i in range(compiled.nx):
        for j in range(compiled.ny):
            cells = np.asarray([int(compiled.grid_to_cell[(i * compiled.ny + j) * compiled.nz + k]) for k in range(compiled.nz)], dtype=np.int64)
            if np.any(cells < 0):
                raise RuntimeError('macro basis requires a complete rectangular macro grid')
...
def assemble(cfg: Package, run: Run) -> Data:
    ...
    return Data(full_layout, detail_steady, detail_transient, PortMap(detail_steady, detail_patches), PortMap(detail_transient, detail_patches), samples, grid_cells(full_layout, 0, cfg.detail_nz), grid_cells(full_layout, cfg.detail_nz, cfg.nz))

def split(sample: Sample):
    p = sample.ports.port_count
    return (sample.K[:p, :p].tocsc(), sample.K[:p, p:].tocsc(), sample.K[p:, :p].tocsc(), sample.K[p:, p:].tocsc(), sample.C[p:, p:].tocsc(), sample.f[:p], sample.f[p:])
