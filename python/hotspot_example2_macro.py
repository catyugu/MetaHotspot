...
Extracts the heat-spreader and heat-sink as a macro-model using BCI-ROM
(Block-Canonical-Insertion ROM).
...
Experiments:
  ...
  2. Transient ROM vs full BE   (uniform T0=318.15K, 10 s, backward Euler,
     periodic piecewise-constant power)
  3. BCI cross-h variation      (basis trained at nominal h, test at
     different convection coefficients)

DOF partition:
  External (e) = TIM + chip layers
  Port     (p) = spreader/sink cells adjacent to TIM (interface)
  Internal (i) = spreader/sink interior cells (the region being reduced)
"""
...
N_TRANSIENT_STEPS = int(TRANSIENT_DURATION / SAMPLING_INTVL)  # 1000

T_AMB = 318.15
H_CONV_NOMINAL = 1.0 / (0.1 * A_SINK)

# Macro region: sink (layer 0) + spreader (layer 1)
MACRO_LAYERS = (0, 1)
MACRO_BLOCK = 0
...
# ====================================================================
#  Helpers  —  model building, ptrace loading, partitioning
# ====================================================================
...
def build_example2_model(
    ...
    Parameters
    ----------
    heat_sources : dict or None
        Block name -> expression (volumetric W/m3).  None = all zero.
    model : metahotspot.Model or None
        Reuse an existing model (for registering functions first).
    h_conv : float or None
        Override convection coefficient.  None = nominal.
    """
    ...
    # Layers (first-added = top → layer 0 = sink)
...
def partition(
    ...
    Port = macro cells adjacent to any non-macro cell.
    Internal = macro cells *not* adjacent to non-macro.
    External = all non-macro cells.

    Returns (e_idx, p_idx, i_idx) — sorted index arrays.
    """
...
def bci_schur_complement(
    ...
    """Compute BCI-ROM Schur complement blocks for a system matrix A.
    ...
    # Projected port block (r×r)
    A_pp_proj = U_r.T @ (A_pp @ U_r)  # (r,r)
    A_ip_U = A_ip @ U_r  # (ni, r)
    T = np.column_stack([A_ii_lu.solve(A_ip_U[:, k]) for k in range(r)])
    correction = U_r.T @ (A_pi @ T)  # (r,r)
...
def bci_steady_solve(
    ...
    """BCI-ROM steady-state solve.

    Parameters
    ----------
    blocks : tuple from bci_schur_complement(), or None to compute fresh.

    Returns full reconstructed temperature field (nc,).
    """
...
def bci_transient_setup(
    ...
    """Pre-compute factorizations for backward-Euler BCI-ROM transient.

    Builds A = C + dt*K then projects via U_r.  Returns a dict of
    cached operators for use in bci_transient_step().
    """
    ne, ni, r = len(e_idx), len(i_idx), U_r.shape[1]
...
def bci_transient_step(
    ...
    Parameters
    ----------
    T_curr : (nc,) — temperature at time n.
    f_np1  : (nc,) — RHS at time n+1.
    ctx    : from bci_transient_setup().

    Returns (nc,) — temperature at time n+1.
    """
    ne, r = ctx["ne"], ctx["r"]
    dt = ctx["dt"]
    C = ctx["C"]
...
def full_transient_solve(
    c: metahotspot.Compiled,
    n_steps: int,
    dt: float,
    T_init: np.ndarray,
    checkpoints: list[int],
) -> dict[int, np.ndarray]:
    """Full backward-Euler transient solve.

    Returns dict: step_index → temperature array (nc,).
    """
    K0 = c.assemble().stiffness_matrix()
    C0 = c.assemble().capacity_matrix()
    A_full = C0 + dt * K0
    A_full_lu = splu(A_full.tocsc())
    ...
    for n in range(n_steps):
        t_np1 = (n + 1) * dt
        f = c.assemble(time=t_np1).rhs()
        rhs = C0 @ T + dt * f
        T = A_full_lu.solve(rhs)
...
def main():
    ...
    K_ref = ref_c.assemble().stiffness_matrix()  # copy, safe to keep
    C_ref = ref_c.assemble().capacity_matrix()
    ...
    n_train = min(100, n_tsteps // 2)
    n_test = min(60, n_tsteps - n_train)
    ...
    for k, ti in enumerate(train_idx):
        ...
        for name in EV6_NAMES:
            ...
            hs[name] = str(ptrace[ti, col] / block_volume[name])
        ...
        print(
            f"  [{k+1:3d}/{n_train}]  "
            f"T_p ∈ [{T[p_idx].min():.2f}, {T[p_idx].max():.2f}]  "
            f"({dt:.2f}s)"
        )
    ...
    # ================================================================
    #  Phase 2: Steady ROM evaluation  (test power patterns, same h)
    # ================================================================
    print("\n" + "=" * 72)
    ...
    print(f"  Pre-computed Schur complement: {offline_s:.3f}s")

    steady_errors = []
    for k, ti in enumerate(test_idx):
        t0 = _time.perf_counter()

        # Build model + compile + assemble RHS + solve full
        hs = {}
        for name in EV6_NAMES:
            col = EV6_NAMES.index(name)
            hs[name] = str(ptrace[ti, col] / block_volume[name])
        ...
        f_test = c.assemble().rhs()  # RHS for THIS power pattern
        sol.close()
        c.close()
        m.close()
        ...
        print(
            f"  [{k+1:3d}/{n_test}]  "
            f"rel.err = {rel:.2e}  max|err| = {mx:.4e}  "
    ...
    print(f"    mean rel. error = {np.mean(rels):.2e}  " f"max = {np.max(rels):.2e}")
    print(f"    mean max|err|   = {np.mean(mxs):.4e}  " f"max = {np.max(mxs):.4e}")

    # ================================================================
    #  Phase 3: Transient ROM  (uniform T0 = 318.15 K, 10 s BE)
    # ================================================================
    print("\n" + "=" * 72)
    ...
    full_check = full_transient_solve(
        c_full_t, N_TRANSIENT_STEPS, SAMPLING_INTVL, T0, checkpts
    )
    ...
    for step in checkpts:
        Ta = full_check[step]
        print(
            f"    t = {step*SAMPLING_INTVL:5.2f}s  "
            f"T ∈ [{Ta.min():.2f}, {Ta.max():.2f}] K"
        )
    c_full_t.close()
    m_full_t.close()
    ...
    c_rom_t.close()
    m_rom_t.close()
    print(f"  ROM transient time: {t_rom:.1f}s  " f"(speedup = {t_full/t_rom:.1f}×)")

    # Transient error table
    print(f"\n  Transient error at checkpoints:")
    print(f"  {'t (s)':>8s}  {'rel.error':>12s}  {'max|err| (K)':>14s}")
    print("  " + "-" * 38)
    for step in sorted(full_check):
        ...
        rel = np.linalg.norm(diff) / np.linalg.norm(Tf)
        mx = np.max(np.abs(diff))
        print(f"  {step*SAMPLING_INTVL:>8.2f}  {rel:>12.2e}  {mx:>14.4e}")
    ...
    # Average power (same as hotspot_example2 steady phase)
    hs_avg = {}
    for name in EV6_NAMES:
        col = EV6_NAMES.index(name)
        hs_avg[name] = str(ptrace[:, col].mean() / block_volume[name])

    for h_test in h_tests:
        t0 = _time.perf_counter()

        # Full reference at this h
        ...
        # ROM solve at this h  (U_r from nominal h; K_ii from this h)
        T_rom_h = bci_steady_solve(
            K_h,
            f_h,
            e_idx,
            p_idx,
        ...
        print(
            f"  h = {h_test:10.2e}  rel.err = {rel:.2e}  "
            f"max|err| = {mx:.4e}  ({_time.perf_counter()-t0:.2f}s)"
        )

    print(f"\n  {'h (W/m2K)':>14s}  {'rel.error':>12s}  " f"{'max|err| (K)':>14s}")
    print("  " + "-" * 42)
    ...
    for s in full_check:
        if s == 0:
            continue
        e = np.linalg.norm(rom_check[s] - full_check[s]) / np.linalg.norm(full_check[s])
        max_trans = max(max_trans, e)

    print("\n" + "=" * 72)
    print("  SUMMARY — Spreader+Sink BCI-ROM macro-model")
    print("=" * 72)
    print(f"  DOFs:  e={ne}  p={np_}  i={ni}  total={nc}")
    print(f"  ROM:   e={ne}  r={r}  total={ne+r}  " f"({100*(ne+r)/nc:.1f}%)")
    print()
    print(f"  Steady ({n_test} test cases):")
    print(f"    mean rel. err = {np.mean(rels):.2e}  " f"max = {np.max(rels):.2e}")
    print()
    print(f"  Transient (10 s, {N_TRANSIENT_STEPS} steps):")
