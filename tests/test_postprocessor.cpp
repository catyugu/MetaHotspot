...
// Helper: build a minimal IOStructure with a non-uniform grid
static IOStructure make_nonuniform_grid_io()
{
    IOStructure io;
    io.study_type = StudyType::Steady;
    io.dimension = Dimension::Dimension3D;
    ...
    // Non-uniform grid: X cells [0,3,10], Y cells [0,4,10], Z cells [0,2,10]
    // dx[0]=3mm, dx[1]=7mm; dy[0]=4mm, dy[1]=6mm; dz[0]=2mm, dz[1]=8mm
    io.mesh_vertex_x = {0.0, 3.0, 10.0};
    io.mesh_vertex_y = {0.0, 4.0, 10.0};
    io.mesh_vertex_z = {0.0, 2.0, 10.0};
...
TEST(PostprocessorTest, NonUniformGridWeightedInterpolation)
{
    // On a non-uniform grid, distance-weighted interpolation should
    // give different results from simple arithmetic mean.
    // Interior node at vertex intersection of 8 cells with unequal spacing.
    auto io = make_nonuniform_grid_io();
    Preprocessor preprocessor;
    auto model = preprocessor.load(io);
    ASSERT_NE(model, nullptr);
    ...
    // Set specific temperatures for each cell
    std::vector<double> cell_T(N);
    for (int i = 0; i < N; i++) {
        cell_T[i] = 300.0 + i * 10.0;
    }

    Postprocessor postprocessor;
    auto node_T = postprocessor.interpolate_cell_to_node(*model, cell_T);

    // Interior node at (vx=1, vy=1, vz=1) - vertex (3mm, 4mm, 2mm)
    int node_ny = model->mesh.ny + 1;
    int node_nz = model->mesh.nz + 1;
    int interior_node = 1 * node_ny * node_nz + 1 * node_nz + 1;

    // Compute expected weighted average using actual model coordinates (in SI meters)
    double node_x = model->mesh.vertex_x[1]; // 3mm in SI
    double node_y = model->mesh.vertex_y[1]; // 4mm in SI
    double node_z = model->mesh.vertex_z[1]; // 2mm in SI
    ...
    for (int ix = 0; ix < 2; ix++) {
        for (int iy = 0; iy < 2; iy++) {
            for (int iz = 0; iz < 2; iz++) {
                int grid_idx = ix * 2 * 2 + iy * 2 + iz;
                int compact_idx = (int)model->cells.index_map[grid_idx];
                double dist = std::sqrt(
                    (model->mesh.cx[ix] - node_x) * (model->mesh.cx[ix] - node_x)
                    + (model->mesh.cy[iy] - node_y) * (model->mesh.cy[iy] - node_y)
                    + (model->mesh.cz[iz] - node_z) * (model->mesh.cz[iz] - node_z));
                double w = 1.0 / dist;
                weighted_sum += w * cell_T[compact_idx];
                weight_sum += w;
            }
        }
    }
    ...
    // The simple average would be 335.0, but weighted should differ
    double simple_avg = (300 + 310 + 320 + 330 + 340 + 350 + 360 + 370) / 8.0;
    EXPECT_NE(expected, simple_avg) << "Weighted interpolation should differ from simple average on non-uniform grid";
