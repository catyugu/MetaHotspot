...
// Helper: build a small in-memory XML with a <TimeScheme> sub-block.
static std::string make_xml_with_time_scheme(const std::string& ts_block)
{
    std::string body = R"(<?xml version="1.0" encoding="utf-8"?>
<Structure>
    <mhs::core::StudyType>Transient</mhs::core::StudyType>
...
TEST(IoTest, SchemeDefaultsToBdf1)
{
    auto path = write_tmp_xml("io_ts_default.xml", make_xml_with_time_scheme(""));
    auto io = mhs::io::read_xml(path.string());
    EXPECT_FALSE(io.has_time_scheme_override);
    EXPECT_EQ(io.time_scheme.kind, mhs::core::TimeSchemeKind::Bdf1);
    std::filesystem::remove(path);
}
...
TEST(IoTest, ParsesAllKnobs)
{
    std::string block = R"(  <Scheme>AdaptiveBdf</Scheme>
  ...
  <OutputDt>0.2</OutputDt>)";
    auto path = write_tmp_xml("io_ts_all.xml", make_xml_with_time_scheme(block));
    auto io = mhs::io::read_xml(path.string());
    EXPECT_TRUE(io.has_time_scheme_override);
    EXPECT_EQ(io.time_scheme.kind, mhs::core::TimeSchemeKind::AdaptiveBdf);
    EXPECT_DOUBLE_EQ(io.time_scheme.initial_dt, 0.05);
    EXPECT_DOUBLE_EQ(io.time_scheme.min_dt, 1e-6);
    EXPECT_DOUBLE_EQ(io.time_scheme.max_dt, 0.5);
    EXPECT_DOUBLE_EQ(io.time_scheme.abs_tol, 1e-7);
    EXPECT_DOUBLE_EQ(io.time_scheme.rel_tol, 1e-4);
    EXPECT_EQ(io.time_scheme.max_order, 2u);
    EXPECT_DOUBLE_EQ(io.time_scheme.output_dt, 0.2);
    std::filesystem::remove(path);
}

TEST(IoTest, OutputDtDefaultsToDuration)
{
    ...
    // Preprocessor (tested separately) will fall back to transient_duration.
    std::filesystem::remove(path);
}
