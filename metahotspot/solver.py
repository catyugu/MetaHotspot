...
from metahotspot.metahotspot_types import SystemMatrix, MeshTopology, PhysicalFields, SimulationResult, ModelConfig
...
def solve_system(sys_mat: SystemMatrix, topo: MeshTopology, fields: PhysicalFields, config: ModelConfig, bcs) -> SimulationResult:
    logger.info("Solving linear system...")
    
    A = sp.csr_matrix((sys_mat.data, sys_mat.col_idx, sys_mat.row_ptr), shape=(sys_mat.n_rows, sys_mat.n_cols))
    ...
    logger.info("Interpolating cell-centered field to nodal mesh...")
    
    # 转换为最终结果节点所需的节点数组 (nx+1, ny+1, nz+1) = xml 中的 SizeX/Y/Z
                        ...
                        for dk in [-1, 0]:
                            ...
                            if 0 <= ci < nx and 0 <= cj < ny and 0 <= ck < nz:
                                if solid_mask[ci, cj, ck]:
                                    val += T_cell_3d[ci, cj, ck]
                                    count += 1
                if count > 0:
                    T_nodes_3d[i, j, k] = val / count

    # 强制覆盖边界节点到其精确的 Dirichlet 定义值
    ...
    for i in range(topo.n_x):
        for j in range(topo.n_y):
            for k in range(topo.n_z):
                if np.isnan(T_nodes_3d[i, j, k]): continue
                
                if i == 0 or i == topo.n_x - 1:
                    bc = find_bc('X', x_m[i], y_m[j], z_m[k], bcs, uc)
                    if bc and bc['type'] == 'first': T_nodes_3d[i, j, k] = bc['temperature']
    ...
    return SimulationResult(
        temperatures=T_flat,  # 返回扁平化后的节点结果
