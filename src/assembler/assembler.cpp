...
namespace mhs::sim {
    ...
    AssemblyResult Assembler::assemble(const AssembleContext& ctx) const
    {
        ...
        // ── Pre-compute compact → old reverse map (shared by both phases) ──
        auto compact_to_old = mhs::utils::buildCompactToOld(cells, total);

        // ═══════════════════════════════════════════════════════════════════
        // Phase 1: 内部传导 + 热源 + 瞬态质量矩阵
        // 遍历所有 cell，对每个面仅在有有效邻居时做内部扩散和对流。
        // 暴露面（无邻居）跳过 —— 由 Phase 2 的面片遍历处理。
        // ═══════════════════════════════════════════════════════════════════
        tbb::parallel_for(0, total, [&](int old_idx) {
            ...
            for (size_t f = 0; f < mhs::core::FACE_COUNT; f++) {
                ...
                double k_neighbor = utils::k_along(dir, mp_n.kx.eval(ctx_n), mp_n.ky.eval(ctx_n), mp_n.kz.eval(ctx_n));

                double d_half_neighbor = mhs::utils::half_length_along(dir, mesh.dx[nix], mesh.dy[niy], mesh.dz[niz]);
        ...
        // ═══════════════════════════════════════════════════════════════════
        // Phase 2: 边界条件面片遍历
        // 对 model_.boundary_patches 做 parallel_for，直接将 BC 积分项
        // 累加到对应的 thread-local 稀疏矩阵和右端向量上。
        // ═══════════════════════════════════════════════════════════════════
        int num_patches = static_cast<int>(patches.size());
        tbb::parallel_for(0, num_patches, [&](int p_idx) {
            const auto& patch = patches[p_idx];
            auto& local = thread_data.local();

            int c_idx = static_cast<int>(patch.cell_idx);
            ...
            const double A_f = mhs::utils::face_area(patch.dir, dx_cell, dy_cell, dz_cell);
            const double half_dist = mhs::utils::half_length_along(patch.dir, dx_cell, dy_cell, dz_cell);
            const double k_face = mhs::utils::k_along(patch.dir, kx_c, ky_c, kz_c);

            switch (patch.type) {
            case mhs::core::BcType::FirstType: {
                double T_bc_val = bc_params.dirichlet_T[patch.param_idx].eval(ctx_c);
            ...
            case mhs::core::BcType::SecondType: {
                double q = bc_params.neumann_q[patch.param_idx].eval(ctx_c);
                local.b(c_idx) += q * A_f;
                break;
            }
            case mhs::core::BcType::ThirdType: {
                double h = bc_params.cauchy_h[patch.param_idx].eval(ctx_c);
                double T_inf = bc_params.cauchy_T_inf[patch.param_idx].eval(ctx_c);
            ...
            default:
                break; // BcType::None (adiabatic) — nothing to assemble
            }
        });
