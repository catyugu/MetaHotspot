...
Reader::Reader(const std::string& xml_path) : xml_path_(xml_path) {}

model::IOStructure Reader::read_structure()
{
    XMLDocument doc;
    XMLError err = doc.LoadFile(xml_path_.c_str());
    if (err != XML_SUCCESS) {
        throw std::runtime_error("Failed to load XML file: " + xml_path_);
    }
