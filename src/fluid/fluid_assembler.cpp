        ...
        tbb::parallel_for(tbb::blocked_range<mhs::Index>(0, total), [&](const tbb::blocked_range<mhs::Index>& range) {
            for (mhs::Index old = range.begin(); old < range.end(); ++old) {
                ...
                for (std::size_t face = 0; face < mhs::core::FACE_COUNT; ++face) {
                    ...
                    if (is_fluid(model.fluid, neighbor)) {
                        ...
                        if (std::abs(mass_flux) <= mhs::core::zero_guard)
                            continue;
                        if (mass_flux > 0.0) {
                            local.emplace_back(static_cast<Eigen::Index>(cell), static_cast<Eigen::Index>(cell),
                                mass_flux * heat_capacity);
                        }
                        else {
                            local.emplace_back(static_cast<Eigen::Index>(cell), static_cast<Eigen::Index>(neighbor),
                ...
                if (net_outflux > 0.0 && !std::isnan(boundary_temperature)) {
                    result.rhs(static_cast<Eigen::Index>(cell)) += net_outflux * heat_capacity * boundary_temperature;
                }
                else {
                    local.emplace_back(
                        static_cast<Eigen::Index>(cell), static_cast<Eigen::Index>(cell), -net_outflux * heat_capacity);
                }
            }
        });
