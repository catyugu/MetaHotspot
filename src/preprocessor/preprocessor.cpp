...
namespace mhs::sim {
    ...
    std::unique_ptr<mhs::core::InternalModel> Preprocessor::load(const mhs::core::IOStructure& ioStructure)
    {
        ...
        // Translate time-scheme spec (defaulting when the XML omits the block).
        // Per ADR-0006 §3.2.2: absent <Scheme> ⇒ Bdf1 + initial_dt = transient_time_step,
        // output_dt = transient_duration.
        if (ioStructure.has_time_scheme_override) {
            model->time_scheme_kind = ioStructure.time_scheme.kind;
            model->ts_initial_dt = ioStructure.time_scheme.initial_dt > 0.0
                                       ? ioStructure.time_scheme.initial_dt
                                       : ioStructure.transient_time_step;
            model->ts_min_dt     = ioStructure.time_scheme.min_dt;
            model->ts_max_dt     = ioStructure.time_scheme.max_dt > 0.0
                                       ? ioStructure.time_scheme.max_dt
            ...
            model->ts_output_dt  = ioStructure.time_scheme.output_dt > 0.0
                                       ? ioStructure.time_scheme.output_dt
                                       : ioStructure.transient_duration;
        }
        else {
