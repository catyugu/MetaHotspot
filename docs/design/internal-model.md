...
扁平 SoA，针对缓存局部性和向量化优化。`src/compiler/runtime_model.hpp`。**所有几何 SI 米**。
...
- **各向异性 k**：`MaterialProps` 按 X / Y / Z 三轴分字段 `kx / ky / kz`，与装配时面法向 1:1 对应。面法向助手统一定义在 `src/compiler/mesh.hpp`。
