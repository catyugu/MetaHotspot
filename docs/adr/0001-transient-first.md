...
- CLAUDE.md explicitly requires native nonlinear treatment, which naturally maps to the transient loop.
...
- **Steady-state solver behavior**: `scheduler` detects `TransientStudyDuration == 0` and runs exactly **one nonlinear iteration** (or iterations until convergence), without any time-stepping loop. This is a direct steady-state solve at t=0.
