...
- The `expr` module exposes a `CompiledExpression` type: compiled expression + symbol table. Evaluated by calling `eval(ctx)` with a `FieldContext`, returning a double.
