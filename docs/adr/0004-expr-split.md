...
**Geometry.** Evaluated by `mhs::core::eval_geometry(formula, symbols)` with muparser against a per-call `SymbolTable` of pre-registered variables.
...
- Block-level heat-source sharing avoids per-cell expression objects.
